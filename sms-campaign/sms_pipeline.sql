...
WITH clean_sms AS (
  SELECT SendDate, TO_HEX(SHA256(CAST(Phone AS STRING))) AS phone_id, Activity,
...
signup_match AS (
  SELECT s.*, emp.signup_date, emp.user_id
  FROM sms s
  LEFT JOIN `user_table` emp
    ON s.phone_id = emp.phone_id
   AND s.SendDate BETWEEN DATE_SUB(emp.signup_date, INTERVAL 30 DAY) AND emp.signup_date
),

lifecycle AS (
  SELECT m.*, r.regist_date,
...
kpi AS (
  SELECT DATE_TRUNC(SendDate, MONTH) AS month,
         ...
         ROUND(COUNT(DISTINCT IF(signed_up=1, phone_id, NULL)) / COUNT(DISTINCT phone_id)*100,1) AS conversion_rate,
         ROUND(COUNT(DISTINCT IF(registered=1, phone_id, NULL)) / COUNT(DISTINCT IF(signed_up=1, phone_id, NULL))*100,1) AS registration_rate
  FROM lifecycle
  GROUP BY 1
),

lead_time AS (
  SELECT DATE_DIFF(regist_date, signup_date, DAY) AS days_to_register
  FROM lifecycle
